...
def verify_real_index_fix():
    with sync_playwright() as p:
        ...
        # Check if scrollbar is scrollable (scrollHeight > clientHeight)
        is_scrollable = page.evaluate("""
            const log = document.getElementById('story-log');
            log.scrollHeight > log.clientHeight
        """)
        print(f"Is scrollable: {is_scrollable}")
        assert is_scrollable, "Story log should have scrollable overflow"

        browser.close()

if __name__ == "__main__":
    verify_real_index_fix()
